...
const FeaturesSection: FC<FeaturesSectionProps> = ({
  ...
  return (
    <section
      id="features"
      className={`text-center pt-12 pb-20 w-full px-4 md:px-8 lg:px-16 bg-background ${className}`}
      data-testid={dataTestId}
    >
      <h2 className="text-4xl font-medium mb-6 text-foreground">{title}</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {features.map((feature, index) => (
          <Card
            key={index}
            className={`p-6 shadow-md bg-background ${feature.className}`}
          >
            <CardHeader>
